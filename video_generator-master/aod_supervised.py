	...
	for k in range(0,1):
		...
		for epoch in range(training_epochs):
		        ...
		        for i in range(total_batch):
		            ...
		            cost_show,my_reonstr,y_predict_show,label_reconstructed_loss,_=sess.run((cost,x_reconstr_mean,y_predict,label_reconstr_loss,optimizer),feed_dict={x_data:batch_xs_data,y_data:batch_xs_label})
		        ...
		        if epoch % display_step == 0:
		            ...
		            train_acc_vec.append(sum(train_predicted_vec==train_real_label)/(network_enc['batch_size']))
...
plt.plot(epoch_vec,train_acc_vec)		       
plt.plot(epoch_vec,test_acc_vec)		       
