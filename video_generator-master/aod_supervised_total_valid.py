...
test_mat = spio.loadmat('total_mean_var_max.mat', squeeze_me=True)
frey_images=test_mat['total_mean_var_max']
rawData=frey_images
...
network_enc={
'n_hidden_recog_1':150, # 1st layer encoder neurons
'n_hidden_recog_2':150, # 2nd layer encoder neuronsn_hidden_gener_1=200, # 1st layer decoder neurons
...
network_dec={
'n_hidden_gen_1':150, # 1st layer encoder neurons
'n_hidden_gen_2':150, # 2nd layer encoder neuronsn_hidden_gener_1=200, # 1st layer decoder neurons
'n_hidden_gener_2':150, # 2nd layer decoder neurons # MNIST data input (img shape: 28*28)
'n_input':rawData.shape[1], # MNIST data input (img shape: 28*28)
'n_data_input':rawData.shape[1],
'n_z':20,
'batch_size':20
...
######################## Z layer ############################
layer_1=tf.nn.softplus(tf.add(tf.matmul(x,x_h1_w),x_h1_b))+0*tf.nn.softplus(tf.add(tf.matmul(y_data,y_h1_w),y_h1_b))
...
label_reconstr_loss = \
    -tf.reduce_sum(y_data * tf.log(1e-2 + y_predict)
                   ...
                   1)
weight_decay=0.005*(tf.nn.l2_loss(layer_1)+tf.nn.l2_loss(layer_2)+tf.nn.l2_loss(layer_1_recons)+tf.nn.l2_loss(layer_2_recons))
# reconstr_loss = \
...
cost=tf.reduce_mean(reconstr_loss + latent_loss+label_reconstr_loss+weight_decay) 
################## Run ##################################
################ Optimizer ###############################
epoch_learning_rate = tf.placeholder_with_default(3e-4, shape=(),name='epoch_learning_rate')
optimizer = \
tf.train.AdamOptimizer(learning_rate=epoch_learning_rate).minimize(cost)
################## Train   ###############################
...
if run==1:
	...
	for k in range(0,1):
		...
		Con_samp_valid=int(round(Validation_size*control_size))
		Pat_samp_valid=int(round(Validation_size*patient_size))
		...
		Index_con_valid= ID_con[Con_samp:Con_samp+Con_samp_valid]
		Index_con_test = ID_con[Con_samp+Con_samp_valid:control_size]

		Index_pat_train = ID_pat[0:Pat_samp]
		Index_pat_valid = ID_pat[Pat_samp:Pat_samp+Pat_samp_valid]
		Index_pat_test = ID_pat[Pat_samp+Pat_samp_valid:patient_size]
		...
		X_valid_con=rawData[Index_con_valid,:]
		X_valid_pat=rawData[Index_pat_valid,:]

		X_train_con_label=rawLabel[Index_con_train,:]
		X_train_pat_label=rawLabel[Index_pat_train,:]

		X_valid_con_label=rawLabel[Index_con_valid,:]
		X_valid_pat_label=rawLabel[Index_pat_valid,:]
		...
		X_valid=np.concatenate((X_valid_con, X_valid_pat), axis=0)
		X_valid_label=np.concatenate((X_valid_con_label, X_valid_pat_label), axis=0)
		...
		# print('X_train label',X_train_label)
		X_valid,X_valid_label=frey_next_batch(X_valid.shape[0],X_valid,X_valid_label)
		X_test,X_test_label=frey_next_batch(X_test.shape[0],X_test,X_test_label)
		# print('X_test shape',X_test.shape)
		# print('X_test label',X_test_label)
		...
		for epoch in range(training_epochs):
		        ...
		        for i in range(total_batch):
		            ...
		            cost_show,my_reonstr,y_predict_show,label_reconstructed_loss,_=sess.run((cost,x_reconstr_mean,y_predict,label_reconstr_loss,optimizer),feed_dict={x_data:batch_xs_data,y_data:batch_xs_label,keep_prob:1,epoch_learning_rate:my_learn_rate})
		        ...
		        if epoch % display_step == 0:
		            ...
		            print('learn_rate',my_learn_rate)
		            train_real_label=X_train_label.ravel(X_train_label.shape[0],)
		            valid_real_label=X_valid_label.ravel(X_valid.shape[0],)
		            y_predict_hat_total_valid=sess.run(y_predict_test,feed_dict={x_test:X_valid})
		            y_predict_total_show,cost_total=sess.run((y_predict,cost),feed_dict={x_data:X_train,y_data:X_train_label})
		            valid_predicted_vec=(y_predict_hat_total_valid>0.5).ravel(X_valid.shape[0],)
		            train_predicted_vec=(y_predict_total_show>0.5).ravel(X_train.shape[0],)
		            test_real_label=X_test_label.ravel(X_test_label.shape[0],)
		            ...
		            print('validation_accuracy',sum(valid_predicted_vec==valid_real_label)/(valid_real_label.shape[0]))
		            ...
		            valid_acc_vec.append(sum(valid_predicted_vec==valid_real_label)/(valid_real_label.shape[0]))
		            ...
		            print('-----------------------------------')
		        if epoch % decision == 0:
		        	my_learn_rate=-((2e-4)/(training_epochs-1))*(epoch-1)+3e-4
		            # plt.subplot(121)
...
ax1.plot(epoch_vec,valid_acc_vec,color='green')		       
ax1.plot(epoch_vec,test_acc_vec,color='red')		       
ax1.grid(True)
fig.legend(['Train accuracy','Validation accuracy','Test accuracyy'])
